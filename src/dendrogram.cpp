...
public:
  ...
  static std::vector<uint32_t> geGlobalLeafIds(const DeNode *root);
...
private:
  ...
  static int32_t mapId(const std::map<int32_t, int32_t> &nodeMap, int32_t key);
...
void DeNode::printLeaf(uint32_t clusterId,
  ...
  printf("cl.node%u \"%s\" %d", clusterId, m_label.c_str(), abs(id));
}
...
void DeNode::printOrder(uint32_t clusterId,
  ...
  if (isLeaf() == true) {
    const int32_t id = mapId(nodeMap, m_id);
    printf("cl.order%u %d \"%s\"", clusterId, abs(id), m_label.c_str());
    return;
  }
...
void DeNode::printStep(uint32_t clusterId,
  ...
  printf("cl.step%u %5d %5d %f  \"%s\" \"%s\"", clusterId, lid, rid, m_dist,
...
Dendrogram::Dendrogram(uint32_t n, const std::vector<std::string> *nodeLables) {
  m_n = n;
  m_dict.resize(m_n * 2);
  ...
  /* generate labels? */
  if (!nodeLables) {
    m_nodeLables.resize(n);
    char buffer[32];
    for (uint32_t i = 0; i < n; i++) {
      m_nodeLables[i] = snprintf(buffer, sizeof(buffer)-1, "%u", i);
    }
  }
  assert(m_nodeLables.size() == n);
  for (uint32_t i = 0; i < m_nodeLables.size(); i++) {
    m_nodes[i] = new DeNode(int32_t(i + 1) * -1, m_nodeLables[i]);
  }
  m_numPairs = 0;
}
